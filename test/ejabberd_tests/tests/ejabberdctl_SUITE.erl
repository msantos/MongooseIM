...
kick_session(Config) ->
    escalus:story(Config, [{alice, 1}], fun(Alice) ->
                ...
                {_, 0} = ejabberdctl("kick_session",
                             [Username, Domain, Resource, "\"Because I can!\""],
                             Config),
...
vcard2_multi_rw(Config) ->
    ...
    {_, 0} = ejabberdctl("set_vcard2_multi",
                      [Username, Domain, "ORG", "ORGUNIT", "'sales;marketing'"],
                      Config),
      ...
      fun(Alice, Bob, Mike, Kate) ->
        ...
        {_, 0} = ejabberdctl("add_rosteritem",
                           [AliceName, Domain, KateName,
                            Domain, "HateHerSheHasSoNiceLegs", "MyGroup", "to"],
                           Config),
...
remove_expired_messages_test(Config) ->
    escalus:story(Config, [{mike, 1}], fun(_) ->
        ...
        OfflineOld = generate_offline_expired_message(JidRecordMike,
                             JidRecordKate, Msg1, OldTimestamp, ExpirationTime),
        ...
        rpc_call(mod_offline_backend, write_messages, [LUser, LServer,
                      [OfflineOld, OfflineNow, OfflineFuture, OfflineFuture2]]),
        %% when
